  ...
  seedArticle({
    ...
    summary: {
      general: {
        ...
        detail:
          "このサンプルは、公式発表を短く読むときの見本です。\n結論で変化の要点を掴んだあと、詳細で背景や注意点を補います。\nすぐに判断が要る内容なら、使える場面と元記事もあわせて確認してください。",
        situations: [
      ...
      engineer: {
        ...
        detail:
          "差分は UI・API・利用制限のどれに効くかを切り分けて読む想定です。\n破壊的変更の有無と、既存連携への影響だけ先に拾います。\n実装判断に進む前に、公式ドキュメントで対象エンドポイントを確認してください。",
        situations: [
  ...
  seedArticle({
    ...
    summary: {
      general: {
        ...
        detail:
          "開発チームの作業の進め方や、納期の見込みに影響しうる更新です。\n自分がコードを書かなくても、『何が楽になったか／注意が増えたか』だけ把握しておくと会議で困りません。\n導入判断はエンジニア向けの詳細と元記事をあわせて見てください。",
        situations: [
      ...
      engineer: {
        ...
        detail:
          "CLI の挙動変更とエージェントの権限・ツール呼び出しまわりを優先して見ます。\nローカル開発環境の設定や既存ワークフローへの影響有無を切り分けてください。\n本番適用前に changelog の該当項目を一度通読するのが安全です。",
        situations: [
  ...
  seedArticle({
    ...
    summary: {
      general: {
        ...
        detail:
          "画面の情報が古い、または更新が反映されないときに疑うポイントの見本です。\n『キャッシュ』と呼ばれる一時保存の仕組みが原因になることがあります。\n問い合わせ対応では、まず再現手順と表示時刻をメモしてからエンジニアへ共有するとスムーズです。",
        situations: [
      ...
      engineer: {
        ...
        detail:
          "キャッシュキー・TTL・再検証のどれが変わったかを先に切り分けます。\n古データや表示遅延の調査では、CDN とアプリ層のどちらで起きているかを確認してください。\n推奨設定が変わっている場合は、既存の ISR / revalidate 設定との差分を見ます。",
        situations: [
          "表示遅延や古データを調査するとき",
          "キャッシュ設定を見直すとき",
          "リリース前に破壊的変更を確認するとき",
